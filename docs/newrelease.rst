...
This is an advanced developer section that describes creation of a new release for pgwall_test and deploying
...
Be sure to update the version in ``pgwall_test/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' pgwall_test/__init__.py
...
From pgwall_test source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- pgwall_test-0.1.0-py2.py3-none-any.whl
    `-- pgwall_test-0.1.0.tar.gz
...
#. From https://github.com/pgwall/pgwall_test click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/pgwall_test and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi pgwall_test
...
.. code-block::
    ...
    pip install pgwall_test


.. _Travis: https://travis-ci.org/pgwall/pgwall_test
.. _main: https://github.com/pgwall/pgwall_test/tree/main
